...
Section "Uninstall"
  ...
  ReadRegDWORD $R4 HKCU "Software\BrowserWorks\${BrandFullNameInternal}" DesktopLauncherAppInstalled
  ${IfNot} ${Errors}
  ${AndIf} $R4 == "1"
    ...
    ${IfNot} ${Errors}
      DeleteRegValue HKCU "Software\BrowserWorks\${BrandFullNameInternal}" DesktopLauncherAppInstalled
    ${EndIf}
  ${EndIf}
  ClearErrors
  ...
  ${Else}
    ...
    StrCpy $RegHive "HKLM"
    DeleteRegValue HKLM "Software\BrowserWorks\${BrandFullNameInternal}" "UpdaterDeletedShortcut"
    ${un.RegCleanMain} "Software\BrowserWorks"
    ${un.RegCleanUninstall}
    ${un.DeleteShortcuts}
  ...
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "BrowserWorks-${AppName}-$AppUserModelID"
