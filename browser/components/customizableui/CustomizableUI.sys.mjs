...
var CustomizableUIInternal = {
  ...
  updateForNewVersion() {
    ...
    if (currentVersion < 23) {
      // Fennec 140 betas registered the blocker button as an external
      ...
      // at its default position via placeNewDefaultWidgetsInArea. Fennec
      // 140 release profiles are version 23 or later and keep their layout.
      for (let area of Object.keys(gSavedState.placements)) {
        let placements = gSavedState.placements[area];
        let idx = placements.indexOf("fennec-blocker-toolbar-button");
        if (idx !== -1) {
