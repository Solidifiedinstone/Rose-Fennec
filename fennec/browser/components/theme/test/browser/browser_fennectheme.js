...
const { FennecTheme } = ChromeUtils.importESModule(
  "resource:///modules/FennecTheme.sys.mjs"
);
const {
...
function assertLoaded(loaded, message) {
  is(FennecTheme.stylesEnabled, loaded, message);
}
...
async function withPaletteSelection(task) {
  ...
  await SpecialPowers.pushPrefEnv({
    set: [
      ...
      [FENNEC_THEME_MODE_PREF, "light"],
      [FENNEC_THEME_COLOR_PREF, "pine"],
    ],
  });
  ...
  await withPaletteSelection(async () => {
    ...
    try {
      const reapplied = TestUtils.topicObserved(
        THEME_UPDATE_TOPIC,
        subject => subject.wrappedJSObject.theme?.id === FENNEC_THEME_ID
      );
      ...
      is(
        ...
        FENNEC_THEME_ID,
        "The Fennec palette replaces the late default theme update"
      );
      is(
      ...
      is(
        updates.filter(id => id === FENNEC_THEME_ID).length,
        1,
        "The Fennec theme update does not loop"
      );
      is(
        updates[updates.length - 1],
        FENNEC_THEME_ID,
        "The Fennec theme is the final update"
      );
    } finally {
...
add_task(async function test_pending_reapply_preserves_new_theme_update() {
  await withPaletteSelection(async () => {
    ...
    try {
      ...
      ok(
        !updates.includes(FENNEC_THEME_ID),
        "The pending Fennec reapply is cancelled"
      );
    } finally {
      Services.obs.removeObserver(observer, THEME_UPDATE_TOPIC);
    }
