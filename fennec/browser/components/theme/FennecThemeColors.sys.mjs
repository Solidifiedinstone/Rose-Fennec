...
export const FENNEC_THEME_MODE_PREF = "browser.theme.fennec.mode";
export const FENNEC_THEME_COLOR_PREF = "browser.theme.fennec.color";
export const FENNEC_THEME_ID = "fennec-dynamic-theme@browserworks.org";
...
const IN_CONTENT_THEME_EXPERIMENT = Object.freeze({
  colors: {
    fennec_accent_primary: "--color-accent-primary",
    fennec_accent_primary_hover: "--color-accent-primary-hover",
    fennec_accent_primary_active: "--color-accent-primary-active",
    fennec_background_color_information: "--background-color-information",
    fennec_icon_color_information: "--icon-color-information",
  },
});

const PALETTES = {
  default: {
    labelId: "fennec-onboarding-color-default-label",
    swatch:
    ...
    content: {
      light: {
        fennec_accent_primary: "#09429f",
        fennec_accent_primary_hover: "#083578",
        fennec_accent_primary_active: "#07295c",
        fennec_background_color_information: "#d8e7f3",
        fennec_icon_color_information: "#09429f",
      },
      dark: {
        fennec_accent_primary: "#b1d3f5",
        fennec_accent_primary_hover: "#c5def6",
        fennec_accent_primary_active: "#d8e7f3",
        fennec_background_color_information: "#07295c",
        fennec_icon_color_information: "#c5def6",
      },
    },
  },
  smoke: {
    labelId: "fennec-onboarding-color-smoke-label",
    swatch:
  ...
  ash: {
    labelId: "fennec-onboarding-color-ash-label",
    swatch:
  ...
  sun: {
    labelId: "fennec-onboarding-color-sun-label",
    swatch:
  ...
  spark: {
    labelId: "fennec-onboarding-color-spark-label",
    swatch:
  ...
  flame: {
    labelId: "fennec-onboarding-color-flame-label",
    swatch:
  ...
  flare: {
    labelId: "fennec-onboarding-color-flare-label",
    swatch:
  ...
  lavender: {
    labelId: "fennec-onboarding-color-lavender-label",
    swatch:
  ...
  dusk: {
    labelId: "fennec-onboarding-color-dusk-label",
    swatch:
  ...
  lagoon: {
    labelId: "fennec-onboarding-color-lagoon-label",
    swatch:
  ...
  tide: {
    labelId: "fennec-onboarding-color-tide-label",
    swatch:
  ...
  pine: {
    labelId: "fennec-onboarding-color-pine-label",
    swatch:
...
function deriveInContentColors(colors, variant) {
  ...
  return {
    fennec_accent_primary: accent,
    fennec_accent_primary_hover: mixHexColor(accent, contrastMixColor, 0.12),
    fennec_accent_primary_active: mixHexColor(accent, contrastMixColor, 0.24),
    fennec_background_color_information: mixHexColor(
      accent,
      subtleMixColor,
      isLight ? 0.88 : 0.62
    ),
    fennec_icon_color_information: accent,
  };
}
...
export const FennecThemeColors = {
  _initialized: false,
  ...
  init() {
    ...
    Services.prefs.addObserver(FENNEC_THEME_MODE_PREF, this);
    Services.prefs.addObserver(FENNEC_THEME_COLOR_PREF, this);
    ...
    if (
      ...
      normalizeColor(Services.prefs.getStringPref(FENNEC_THEME_COLOR_PREF)) ==
        DEFAULT_COLOR
    ) {
      Services.prefs.clearUserPref(FENNEC_THEME_COLOR_PREF);
    }
  ...
  uninit() {
    ...
    Services.prefs.removeObserver(FENNEC_THEME_MODE_PREF, this);
    Services.prefs.removeObserver(FENNEC_THEME_COLOR_PREF, this);
  ...
  hasSelection() {
    return (
      Services.prefs.prefHasUserValue(FENNEC_THEME_MODE_PREF) ||
      Services.prefs.prefHasUserValue(FENNEC_THEME_COLOR_PREF)
    );
  },
  ...
  getMode() {
    return normalizeMode(
      Services.prefs.getStringPref(FENNEC_THEME_MODE_PREF, DEFAULT_MODE)
    );
  },

  getColor() {
    return normalizeColor(
      Services.prefs.getStringPref(FENNEC_THEME_COLOR_PREF, DEFAULT_COLOR)
    );
  },

  setMode(mode) {
    Services.prefs.setStringPref(FENNEC_THEME_MODE_PREF, normalizeMode(mode));
    return this.apply();
  },
  ...
  setColor(color) {
    ...
    if (color == DEFAULT_COLOR) {
      if (Services.prefs.prefHasUserValue(FENNEC_THEME_COLOR_PREF)) {
        Services.prefs.clearUserPref(FENNEC_THEME_COLOR_PREF);
      }
      if (this.hasSelection()) {
        return this.apply();
    ...
    Services.prefs.setStringPref(FENNEC_THEME_COLOR_PREF, color);
    return this.apply();
  },
  ...
  clear() {
    for (let pref of [FENNEC_THEME_MODE_PREF, FENNEC_THEME_COLOR_PREF]) {
