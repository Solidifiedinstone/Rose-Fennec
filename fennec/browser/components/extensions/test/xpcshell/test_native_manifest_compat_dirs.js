...
const USER_APP_DIR_NAME =
  AppConstants.platform === "linux" ? ".fennec" : "Fennec";
...
let dir = FileUtils.getDir("TmpD", ["NativeManifestsFennec"]);
...
add_task(async function test_mozilla_user_dir_precedence() {
  ...
  await writeApplicationManifest(
    globalDir,
    "compat_precedence",
    "This manifest is from the Fennec system directory"
  );
  ...
  equal(
    result.path,
    user.path,
    "lookupApplication returns the Mozilla user path before the Fennec system path"
  );
  deepEqual(
    result.manifest,
    user.manifest,
...
add_task(async function test_fennec_user_dir_precedence() {
  let fennec = await writeApplicationManifest(
    userDir,
    "fennec_precedence",
    "This manifest is from the Fennec user directory"
  );
  await writeApplicationManifest(
    userCompatDir,
    "fennec_precedence",
  ...
  let result = await lookupApplication("fennec_precedence");
  notEqual(
    result,
    null,
    "lookupApplication finds a Fennec manifest before a Mozilla manifest"
  );
  equal(
    result.path,
    fennec.path,
    "lookupApplication returns the Fennec user path before the Mozilla user path"
  );
  deepEqual(
    result.manifest,
    fennec.manifest,
    "lookupApplication returns the Fennec user manifest contents"
  );
});
