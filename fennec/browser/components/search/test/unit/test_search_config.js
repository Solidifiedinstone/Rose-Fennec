...
const {
  getFennecDefaultSearchEngineId,
...
add_task(async function test_static_search_data() {
  ...
  Assert.deepEqual(
    ...
    ENGINE_IDS,
    "The static engine list should contain the Fennec engines"
  );
  Assert.deepEqual(
    [...icons.keys()],
    ENGINE_IDS,
    "Every Fennec engine should have a static icon"
  );
  Assert.ok(
    !Object.hasOwn(
...
add_task(function test_qwant_default_regions() {
  for (const region of ["US", "fr", "KR"]) {
    Assert.equal(
      getFennecDefaultSearchEngineId(region),
      "qwant",
