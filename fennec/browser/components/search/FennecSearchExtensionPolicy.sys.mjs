...
const POLICY_HIDDEN_ATTR = "fennecHiddenForAdClickExtensions";
const POLICY_SWITCHED_FROM_ATTR = "fennecAdClickExtensionSwitchedFrom";
...
const ATTRIBUTION_PREFS = Object.freeze({
  "1org": "browser.search.param.fennec_attribution_1org",
  ddg: "browser.search.param.fennec_attribution_ddg",
  ecosia: "browser.search.param.fennec_attribution_ecosia",
  qwant: "browser.search.param.fennec_attribution_qwant",
});
 ...
 */
export const FennecSearchExtensionPolicy = {
  _addonListener: {
    onEnabled(addon) {
      FennecSearchExtensionPolicy._onAddonStateChanged(addon);
    },
    onDisabled(addon) {
      FennecSearchExtensionPolicy._onAddonStateChanged(addon);
    },
    onInstalled(addon) {
      FennecSearchExtensionPolicy._onAddonStateChanged(addon, "installed");
    },
    onUninstalled(addon) {
      FennecSearchExtensionPolicy._onAddonStateChanged(addon);
    },
  ...
  init() {
    ...
    this._refreshPolicy(true).catch(error =>
      console.error(
        "FennecSearchExtensionPolicy startup refresh failed",
        error
      )
    );
  },
  ...
  _onAddonStateChanged(addon, eventName) {
    ...
    if (eventName == "installed" && this._isAdClickExtension(addon)) {
      ...
      this._applyActivePolicy(false).catch(error =>
        console.error(
          "FennecSearchExtensionPolicy failed to apply policy",
          error
        )
      );
      return;
    ...
    this._refreshTimer = lazy.setTimeout(() => {
      ...
      this._refreshPolicy(false).catch(error =>
        console.error(
          "FennecSearchExtensionPolicy failed to refresh policy",
          error
        )
      );
  ...
  async _maybeSwitchDefaultToFallback(privateMode, isStartup) {
    ...
    if (!fallbackEngine) {
      console.warn(
        "FennecSearchExtensionPolicy found no neutral fallback engine"
      );
      return;
    }
