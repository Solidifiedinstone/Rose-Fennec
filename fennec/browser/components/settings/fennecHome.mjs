...
function wrapHomepageNewTabs(config) {
  if (config._fennecCustomNewTab) {
    return;
  }
  config._fennecCustomNewTab = true;
  ...
  config.getControlConfig = (controlConfig, deps, setting) => {
    ...
    return {
      ...
      options: [
        ...
        {
          value: "custom",
          l10nId: "fennec-home-mode-choice-custom-new-tab-url",
        },
        ...rest,
      ],
    };
...
const CUSTOM_NEWTAB_ITEM = {
  ...
  l10nId: "fennec-home-new-tab-custom-url",
  controlAttrs: {
  ...
  items: [
    {
      ...
      l10nId: "fennec-home-new-tab-custom-url-input",
    },
  ],
};
...
function injectCustomNewTabItem(config) {
  ...
  if (config.items.some(item => item.id === "fennecCustomNewTabUrlGroup")) {
    return;
  }
...
// Fennec ships no sponsored content, so drop the "Support Firefox" sponsored
...
// Firefox brand; Fennec uses neutral wording.
const HOME_LABEL_OVERRIDES = {
  "home-mode-choice-default-fx-srd": "fennec-home-mode-choice-default",
  "home-prefs-content-header": "fennec-home-prefs-content-header",
  "home-prefs-firefox-home-disabled-notice": "fennec-home-disabled-notice",
};
