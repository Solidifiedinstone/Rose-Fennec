...
add_task(async function test_tabs_group_renders() {
  ...
  let group = await settingGroupRenders(doc, "fennecTabs");
  ok(group, "The Fennec tabs group renders on the tabs pane");

  let select = doc
    .getElementById("setting-control-fennec-tab-bar-position")
...
add_task(async function test_menu_toggle_writes_pref() {
  ...
  await settingGroupRenders(doc, "fennecTabs");
  let toggle = doc
    .getElementById("setting-control-fennec-tabs-duplicate-menu")
...
add_task(async function test_grouping_placement_follows_master() {
  ...
  await settingGroupRenders(doc, "fennecTabs");
  let placement = doc
    .getElementById("setting-control-fennec-auto-group-placement")
    ?.querySelector("moz-select");
  ...
  let toggle = doc
    .getElementById("setting-control-fennec-auto-group-tabs")
    ?.querySelector("moz-toggle");
  synthesizeClick(toggle);
  await prefChanged;
