...
const ENABLED_PREF = "fennec.blocker.enabled";
...
add_task(async function test_ad_blocking_pane_registers() {
  ...
  await settingGroupRenders(doc, "fennecBlocker");
  for (let groupId of [
    "fennecBlocker",
    "fennecBlockerLists",
...
add_task(async function test_master_toggle_writes_pref() {
  await SpecialPowers.pushPrefEnv({
    set: [
      ["fennec.blocker.enabledLists", DISABLED_LIST_OVERRIDES],
      ["fennec.blocker.filterListUrls", "[]"],
      ["fennec.blocker.remoteResourcesEnabled", false],
      [ENABLED_PREF, true],
    ],
  });
  ...
  let control = doc.getElementById("setting-control-fennec-blocker-enabled");
...
add_task(async function test_pane_hidden_without_blocker_ui() {
  await SpecialPowers.pushPrefEnv({
    set: [["fennec.blocker.ui.enabled", false]],
  });
