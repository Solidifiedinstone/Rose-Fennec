...
function getFallbackSelect(doc) {
  let select = doc
    .getElementById("setting-control-fennec-ultra-fallback")
...
add_task(async function test_ultra_is_separate_radio_option() {
  ...
  try {
    ...
    is(
      ...
      "The Ultra option has its own Fennec label"
    );
    ok(
      !doc.querySelector('setting-group[groupid="fennecUltraDns"]'),
...
add_task(async function test_ultra_details_and_status_use_ohttp_model() {
  ...
  try {
    ...
    await waitForBoxItemDescription(doc, "fennec-ultra-relay-uri", relay);
    await waitForBoxItemDescription(
      doc,
      "fennec-ultra-endpoint-uri",
      endpoint
    );
    ...
    await TestUtils.waitForCondition(
      ...
      "Ultra uses a green Fennec/OHTTP status message"
    );
    let args = JSON.parse(status.getAttribute("data-l10n-args"));
    is(args.relay, "Fennec", "The Ultra status includes the relay name");
    is(
      args.provider,
      "Cloudflare",
...
add_task(async function test_selecting_custom_leaves_ultra() {
  ...
  try {
    ...
    await TestUtils.waitForCondition(
      () =>
        !getStatusBar(doc).getAttribute("data-l10n-id").startsWith("fennec-"),
