...
add_task(async function test_appearance_group_renders() {
  ...
  let group = await settingGroupRenders(doc, "fennecAppearance");
  ok(group, "The Fennec styling group renders on the appearance pane");

  let select = doc
    .getElementById("setting-control-fennec-lepton-mode")
  ...
  let toggleControl = doc.getElementById(
    "setting-control-fennec-appearance-autohide-tabbar"
  );
...
add_task(async function test_preset_select_writes_block() {
  ...
  await settingGroupRenders(doc, "fennecAppearance");
  let select = doc
    .getElementById("setting-control-fennec-lepton-preset")
    ?.querySelector("moz-select");
