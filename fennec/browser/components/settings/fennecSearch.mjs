...
// under the address bar header. Fennec locks Suggest off, so these never apply
...
// enabled; Fennec uses a neutral "Suggestions" label instead.
const MOZ_SUGGEST_HEADER_L10N_ID = "addressbar-header-firefox-suggest-2";
const FENNEC_SUGGEST_HEADER_L10N_ID =
  "fennec-addressbar-header-suggestions";

const ADDRESS_BAR_BEHAVIOR_ITEM = {
  id: "fennecAddressBarBehavior",
  l10nId: "fennec-search-address-bar-behavior-heading",
  ...
  items: [
    {
      id: "fennecClickSelectsAll",
      l10nId: "fennec-search-click-selects-all-toggle",
      control: "moz-toggle",
      controlAttrs: {
    ...
    {
      id: "fennecDoubleClickSelectsAll",
      l10nId: "fennec-search-double-click-selects-all-toggle",
      control: "moz-toggle",
      controlAttrs: {
...
Preferences.addSetting({ id: "fennecAddressBarBehavior" });
...
function appendAddressBarBehavior(group) {
  if (group.items.some(item => item.id === "fennecAddressBarBehavior")) {
    return;
  }
...
function wrapSuggestHeaderLabel(config) {
  if (config._fennecGenericLabel) {
    return;
  }
  config._fennecGenericLabel = true;
  ...
  config.getControlConfig = (...args) => {
    ...
    if (result?.l10nId === MOZ_SUGGEST_HEADER_L10N_ID) {
      return { ...result, l10nId: FENNEC_SUGGEST_HEADER_L10N_ID };
    }
    return result;
  };
}
