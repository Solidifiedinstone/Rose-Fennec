...
const DOH_GROUP_BLURBS = {
  dnsOverHttps: {
    ...
    ultraL10nId: "fennec-doh-group-ultra",
  },
  dnsOverHttpsAdvanced: {
    standardL10nId: "preferences-doh-advanced-section",
    ultraL10nId: "fennec-doh-advanced-section-ultra",
  },
};
...
function ultraIsActiveFromDeps(deps) {
  return ultraStateActive(
    deps.fennecUltraOhttp.value,
    deps.fennecUltraUseGet.value,
    deps.dohMode.value,
    deps.dohURL.value
  );
}
...
function getUltraStatusArgs(deps) {
  return {
    relay: displayNameFromURI(
      getStringSettingValue(deps, "fennecUltraRelayUri", OHTTP_RELAY_PREF),
      {
        "dooh.waterfox.net": "Fennec",
      }
    ),
    provider: displayNameFromURI(
      getStringSettingValue(
        deps,
        "fennecUltraEndpointUri",
        OHTTP_ENDPOINT_PREF
      ),
      {
...
function getUltraStatusConfig(config, deps) {
  return {
    ...config,
    l10nId: "fennec-doh-status-ultra-active",
...
function ultraRadioOption() {
  return {
    id: "dohRadioUltra",
    value: "ultra",
    l10nId: "fennec-doh-radio-ultra",
    items: [
      {
        id: "fennec-ultra-fallback",
        l10nId: "fennec-ultra-fallback-select",
        control: "moz-select",
        options: [
          {
            value: "fallback",
            l10nId: "fennec-ultra-fallback-option-allowed",
          },
          {
            value: "no-fallback",
            l10nId: "fennec-ultra-fallback-option-disabled",
          },
        ],
      },
      {
        id: "fennec-ultra-relay-uri",
        control: "moz-box-item",
        l10nId: "fennec-doh-ultra-relay",
      },
      {
        id: "fennec-ultra-endpoint-uri",
        control: "moz-box-item",
        l10nId: "fennec-doh-ultra-endpoint",
      },
    ],
  };
}
...
const DOH_SETTING_WRAPPERS = {
  dohModeBoxItem: {
    deps: ["fennecUltraOhttp", "fennecUltraUseGet", "dohURL"],
    wrap(config) {
      ...
      config.getControlConfig = (controlConfig, deps, setting) => {
        ...
        return ultraIsActiveFromDeps(deps)
          ? { ...result, l10nId: "fennec-doh-overview-ultra" }
          : result;
      };
    },
  },
  ...
  dohRadioGroup: {
    deps: ["fennecUltraOhttp", "fennecUltraUseGet"],
    wrap(config) {
...
Preferences.addSetting({
  id: "fennecUltraUseGet",
  pref: USE_GET_PREF,
});

Preferences.addSetting({
  id: "fennecUltraRelayUri",
  pref: OHTTP_RELAY_PREF,
});

Preferences.addSetting({
  id: "fennecUltraEndpointUri",
  pref: OHTTP_ENDPOINT_PREF,
});

Preferences.addSetting({
  id: "fennec-ultra-enabled",
  deps: ["fennecUltraOhttp", "fennecUltraUseGet", "dohMode", "dohURL"],
  get: (_val, deps) =>
    ultraStateActive(
      deps.fennecUltraOhttp.value,
      deps.fennecUltraUseGet.value,
...
Preferences.addSetting({
  id: "fennec-ultra-fallback",
  deps: ["fennec-ultra-enabled", "dohMode"],
  get: (_val, deps) =>
  ...
  disabled: deps => !deps["fennec-ultra-enabled"].value,
});

for (let [id, pref] of [
  ["fennec-ultra-relay-uri", OHTTP_RELAY_PREF],
  ["fennec-ultra-endpoint-uri", OHTTP_ENDPOINT_PREF],
]) {
  Preferences.addSetting({
    id,
    pref,
