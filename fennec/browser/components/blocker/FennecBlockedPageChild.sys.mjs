 ...
 */
export class FennecBlockedPageChild extends JSWindowActorChild {
  handleEvent(event) {
    ...
    this.sendQuery("FennecBlockedPage:AllowAndNavigate", {
      url: blockedUrl,
    })
      .then(ok => {
        if (!ok) {
      ...
      .catch(err => {
        console.error(
          "[FennecBlockedPageChild] AllowAndNavigate failed:",
          err
        );
      });
  }
