...
import { FennecBlockerService } from "resource:///modules/FennecBlockerService.sys.mjs";
import {
  isPrivateBrowsingContext,
  toSafeDomain,
} from "resource:///modules/FennecBlockerUtils.sys.mjs";
...
const PREF_BRANCH = "fennec.blocker.";
const PREF_ENABLED = "fennec.blocker.enabled";
const PREF_UI_ENABLED = "fennec.blocker.ui.enabled";
const PREF_SHOW_BADGE = "fennec.blocker.showBadge";
...
const TOPIC_BLOCKED_COUNT_UPDATED = "FennecBlocker:BlockedCountUpdated";
const TOPIC_BLOCKED_COUNTS_CLEARED = "FennecBlocker:BlockedCountsCleared";
...
const WIDGET_ID = "fennec-blocker-toolbar-button";
const PANEL_STYLESHEET_URI =
  "chrome://browser/content/blocker/fennecBlockerPanel.css";
const HERO_LOGO_URI = "chrome://browser/content/blocker/fennecShield.svg";

const PANEL_IDS = {
  panel: "fennec-blocker-panel",
  multiview: "fennec-blocker-multiview",
  mainView: "fennec-blocker-mainView",
  blockedListView: "fennec-blocker-blockedListView",
  hero: "fennec-blocker-hero",
  heroTitle: "fennec-blocker-hero-title",
  heroSubtitle: "fennec-blocker-hero-subtitle",
  categories: "fennec-blocker-categories",
  categoriesFooter: "fennec-blocker-categories-footer",
  seeAllButton: "fennec-blocker-see-all",
  pausedCard: "fennec-blocker-paused-card",
  toggleRow: "fennec-blocker-toggle-row",
  siteToggle: "fennec-blocker-panel-site-toggle",
  allowlistRow: "fennec-blocker-allowlist-row",
  allowlistCount: "fennec-blocker-allowlist-count",
  footerStats: "fennec-blocker-footer-stats",
  settingsButton: "fennec-blocker-settings-button",
  detailBackButton: "fennec-blocker-detail-back",
  detailSubtitle: "fennec-blocker-detail-subtitle",
  detailCountPill: "fennec-blocker-detail-count",
  detailBody: "fennec-blocker-detail-body",
};

const L10N_IDS = {
  notAvailable: "fennec-blocker-panel-not-available",
  disabled: "fennec-blocker-panel-disabled",
  partnerAllowed: "fennec-blocker-panel-partner-allowed",
  heroCount: "fennec-blocker-panel-hero-count",
  heroPaused: "fennec-blocker-panel-hero-paused",
  heroSubtitle: "fennec-blocker-panel-hero-subtitle",
  pausedCard: "fennec-blocker-panel-paused-card",
  seeAll: "fennec-blocker-panel-see-all",
  toggle: "fennec-blocker-panel-toggle2",
  allowlist: "fennec-blocker-panel-allowlist",
  allowlistCount: "fennec-blocker-panel-allowlist-count",
  footerStats: "fennec-blocker-panel-footer-stats",
  footerSettings: "fennec-blocker-panel-footer-settings",
  back: "fennec-blocker-panel-back",
  detailTitle: "fennec-blocker-panel-detail-title",
  allowDomain: "fennec-blocker-panel-allow-domain",
  domainCount: "fennec-blocker-panel-domain-count",
  popupNote: "fennec-blocker-panel-detail-popup-note",
};

const CATEGORIES = [
  {
    key: "ads",
    rowL10nId: "fennec-blocker-panel-category-ads",
    sectionL10nId: "fennec-blocker-panel-detail-section-ads",
  },
  {
    key: "trackers",
    rowL10nId: "fennec-blocker-panel-category-trackers",
    sectionL10nId: "fennec-blocker-panel-detail-section-trackers",
  },
  {
    key: "popups",
    rowL10nId: "fennec-blocker-panel-category-popups",
    sectionL10nId: "fennec-blocker-panel-detail-section-popups",
  },
];
...
/**
 * Owns the Fennec blocker toolbar button and popup panel.
 *
 ...
 * and routes interactions to `FennecBlockerService`.
 */
export const FennecBlockerPanel = {
  _initialized: false,
  ...
  _handleTabOpen(win, event) {
    ...
    FennecBlockerService.recordNewTabSourceHost(
      newBrowserId,
  ...
  _hookBrowserWindow(win) {
    ...
    const onLocationChange = (
      ...
      if (isTopLevel && !isSameDocument) {
        ...
        if (browserId) {
          FennecBlockerService.resetBlockedCount(browserId);
        }
      }
  ...
  _showBlockedListView(win) {
    ...
    try {
      ...
      console.error("[FennecBlockerPanel] Failed to open subview:", err);
    }
  },
  ...
  _readBlockedCount(browserId) {
    ...
    return Number(FennecBlockerService.getBlockedCount(browserId) || 0);
  },
  ...
  _refreshFooter(doc) {
    ...
    const globalStats = FennecBlockerService.getGlobalStats();
  ...
  _refreshAllowlistRow(doc) {
    ...
    setNodeL10nAttributes(doc, count, L10N_IDS.allowlistCount, {
      count: FennecBlockerService.getSiteExceptionCount(),
    });
  },
  ...
  _buildDomainRow(doc, win, host, entry) {
    ...
    if (!FennecBlockerService.isDomainExceptedOnSite(host, entry.domain)) {
      ...
      allowButton.addEventListener("click", () => {
        FennecBlockerService.addDomainExceptionForSite(host, entry.domain);
        this._refreshBlockedListView(win);
      });
  ...
  _refreshBlockedListView(win) {
    ...
    const stats = FennecBlockerService.getBlockedStats(browserId);
  ...
  _refreshPanelForWindow(win, blockedCount, enabled) {
    ...
    const excepted = host
      ? FennecBlockerService.isSiteExcepted(host, options)
      : false;
    const partnerBypass =
      activeEnabled &&
      protectable &&
      !excepted &&
      FennecBlockerService.shouldBypassBlocking(host, options);
  ...
  _setSiteExceptionForCurrentSite(win, disableForSite) {
    ...
    if (this._isPrivateWindow(win)) {
      if (disableForSite) {
        FennecBlockerService.allowSiteForSession(host, { isPrivate: true });
      } else {
        FennecBlockerService.removeSiteException(host, { isPrivate: true });
      }
    } else if (disableForSite) {
      FennecBlockerService.addSiteException(host);
    } else {
      FennecBlockerService.removeSiteException(host);
    }
  ...
  uninit() {
    ...
    for (const topic of OBSERVED_TOPICS) {
      try {
        ...
        console.warn(
          `[FennecBlockerPanel] Failed to remove observer for ${topic}:`,
          err
        );
      }
    }
    ...
    try {
      ...
      console.warn(
        "[FennecBlockerPanel] Failed to remove pref observer:",
        err
      );
    }
