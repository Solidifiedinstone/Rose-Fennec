...
const BLOCKER_ENABLED_PREF = "fennec.blocker.enabled";
const COSMETIC_STYLE_ID = "fennec-blocker-cosmetic-style";
const EARLY_SCRIPTLET_DIAGNOSTICS_PREF =
  "fennec.blocker.earlyScriptletInjection.diagnostics";
const EARLY_SCRIPTLET_PARSER_BLOCK_TIMEOUT_PREF =
  "fennec.blocker.earlyScriptletInjection.parserBlockTimeoutMs";
 ...
 */
export class FennecBlockerChild extends JSWindowActorChild {
  /**
  ...
  receiveMessage(message) {
    try {
      switch (message.name) {
        case "FennecBlocker:ApplyCosmeticSelectors":
          return this._applyCosmeticSelectors(message.data?.selectors || []);

        case "FennecBlocker:ClearCosmeticSelectors":
          this._clearCosmeticSelectors();
          return { ok: true };

        case "FennecBlocker:CollectClassIdSnapshot":
      ...
      console.error("[FennecBlockerChild] receiveMessage failed:", err);
  ...
  _createEarlyScriptletBlocker() {
    ...
    const finish = (path = "aborted") => {
      ...
      if (Services.prefs.getBoolPref(EARLY_SCRIPTLET_DIAGNOSTICS_PREF, false)) {
        try {
          ...
          pageWindow.__fennecBlockerEarlyScriptletDiagnostics = {
    ...
    try {
      ...
      console.error(
        "[FennecBlockerChild] failed to block parser for early scriptlet:",
        err
      );
      finish("block-error");
      return null;
  ...
  _initializeForCurrentDocument(eventName, earlyScriptletBlocker = null) {
    ...
    this._onDOMWindowCreated({ earlyScriptletBlocker })
      ...
      .catch(err => {
        console.error(
          `[FennecBlockerChild] ${eventName} initialization failed:`,
          err
        );
      })
      .finally(() => {
  ...
  async _onDOMWindowCreatedInner({ earlyScriptletBlocker = null } = {}) {
    ...
    try {
      ...
      console.error("[FennecBlockerChild] failed to get URL:", err);
      return;
    }
    ...
    let resources;
    try {
      resources = await this.sendQuery("FennecBlocker:GetCosmeticResources", {
        url,
      });
    } catch (err) {
      console.error(
        "[FennecBlockerChild] sendQuery(GetCosmeticResources) failed:",
        err
      );
  ...
  didDestroy() {
    ...
    try {
      ...
      console.error(
        "[FennecBlockerChild] failed clearing cosmetic selectors during destroy:",
        err
      );
    }
  }
  ...
  _clearResourceRetryTimeout() {
    ...
    try {
      ...
      console.error(
        "[FennecBlockerChild] failed to clear resource retry timeout:",
        err
      );
    }
  ...
  _scheduleInitialResourcesRetry(targetDocument) {
    ...
    const timeoutId = this.contentWindow?.setTimeout(() => {
      ...
      this._retryInitialResources(targetDocument).catch(err => {
        console.error(
          "[FennecBlockerChild] delayed cosmetic resources retry failed:",
          err
        );
      });
  ...
  async _retryInitialResources(targetDocument) {
    ...
    try {
      ...
      console.error(
        "[FennecBlockerChild] delayed sendQuery(GetCosmeticResources) failed:",
        err
      );
      return;
    }
  ...
  _flushInitialResources() {
    ...
    if (resources.injectedScript) {
      this._injectScriptlet(resources.injectedScript).catch(err => {
        console.error(
          "[FennecBlockerChild] failed to inject scriptlet:",
          err
        );
      });
    }
  ...
  _teardownProceduralActionObserver() {
    if (this._proceduralObserver) {
      try {
        ...
        console.error(
          "[FennecBlockerChild] failed to disconnect procedural observer:",
          err
        );
      }
    ...
    if (this._proceduralMutationTimeout) {
      try {
        ...
        console.error(
          "[FennecBlockerChild] failed to clear procedural timeout:",
          err
        );
      }
      this._proceduralMutationTimeout = null;
  ...
  async _queryAndApplyNewSelectors(classes, ids) {
    ...
    try {
      selectors = await this.sendQuery(
        "FennecBlocker:GetHiddenClassIdSelectors",
        {
          classes,
      ...
      console.error(
        "[FennecBlockerChild] sendQuery(GetHiddenClassIdSelectors) failed:",
        err
      );
      return;
    }
...
`;
    ...
    try {
      ...
      script = await ChromeUtils.compileScript(scriptUrl, {
        filename: "fennec-blocker-scriptlet.js",
      });
    } catch (err) {
      console.error("[FennecBlockerChild] failed to compile scriptlet:", err);
      return false;
    }
    ...
    try {
      ...
      console.error("[FennecBlockerChild] failed to inject scriptlet:", err);
      return false;
    }
  }
  ...
  _teardownGenericHideObserver() {
    if (this._observer) {
      try {
        ...
        console.error(
          "[FennecBlockerChild] failed to disconnect generic-hide observer:",
          err
        );
      }
      this._observer = null;
    ...
    if (this._mutationTimeout) {
      try {
        ...
        console.error(
          "[FennecBlockerChild] failed to clear mutation timeout:",
          err
        );
      }
      this._mutationTimeout = null;
    ...
    if (this._initialCollectTimeout) {
      try {
        ...
        console.error(
          "[FennecBlockerChild] failed to clear initial collection timeout:",
          err
        );
      }
      this._initialCollectTimeout = null;
