...
ChromeUtils.defineLazyGetter(lazy, "blockerLocalization", () => {
  return new Localization(["browser/fennec/blocker.ftl"]);
});

const PREF_BLOCKER_ENABLED = "fennec.blocker.enabled";
const PREF_BLOCKER_UI_ENABLED = "fennec.blocker.ui.enabled";
...
const PREF_COEXIST = "fennec.blocker.coexist";

const DETECTION_DELAY_MS = 500;
const CHROME_DOCUMENT_LOADED_TOPIC = "chrome-document-loaded";
const PREF_CHANGED_TOPIC = "nsPref:changed";
const EXTENSION_NAME_PLACEHOLDER = "__FENNEC_BLOCKER_EXTENSION_NAME__";
const L10N_ID_EXTENSION_FALLBACK_NAME_THIS =
  "fennec-blocker-extension-fallback-name-this";
const L10N_ID_EXTENSION_FALLBACK_NAME_YOUR =
  "fennec-blocker-extension-fallback-name-your";
const L10N_ID_INSTALL_WARNING = "fennec-blocker-extension-install-warning";
const L10N_ID_INSTALL_WARNING_MANAGE_SETTINGS =
  "fennec-blocker-extension-install-manage-settings";
const L10N_ID_PROMPT_TITLE = "fennec-blocker-prompt-title";
const L10N_ID_INSTALL_ANYWAY = "fennec-blocker-extension-install-anyway";
 ...
 * warnings read from.
 */
export const FennecBlockerExtensionDetector = {
  _addonListener: {
    onEnabled(addon) {
      FennecBlockerExtensionDetector._onAddonStateChanged(addon);
    },
    onDisabled(addon) {
      FennecBlockerExtensionDetector._onAddonStateChanged(addon);
    },
  ...
  _installListener: {
    onInstallStarted(install) {
      return FennecBlockerExtensionDetector._onInstallStarted(install);
    },
    onInstallEnded(install) {
      FennecBlockerExtensionDetector._onInstallEnded(install);
    },
  },
  ...
  async _preloadLocalizedStrings() {
    ...
    const loadPromise = (async () => {
      ...
      this._localizedStringCache.set(
        "fennec-blocker-extension-install-warning-template",
  ...
  init() {
    ...
    this._preloadLocalizedStrings().catch(error => {
      console.error(
        "[FennecBlockerExtensionDetector] Failed to preload localized strings",
        error
      );
    });
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[FennecBlockerExtensionDetector] Startup detection failed",
        error
      );
    });
  },
  ...
  uninit() {
    ...
    try {
      ...
      console.warn(
        "[FennecBlockerExtensionDetector] Failed to remove enabled observer:",
        err
      );
    }
    ...
    try {
      ...
      console.warn(
        "[FennecBlockerExtensionDetector] Failed to remove dismissed observer:",
        err
      );
    }
    ...
    try {
      ...
      console.warn(
        "[FennecBlockerExtensionDetector] Failed to remove coexist observer:",
        err
      );
    }
  },
  ...
  _onAddonStateChanged(addon) {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[FennecBlockerExtensionDetector] Failed to refresh detection notifications",
        error
      );
    });
  },
  ...
  _onBuiltInBlockerPrefChanged() {
    ...
    this._handleBuiltInBlockerReenabled()
      ...
      .catch(error => {
        console.error(
          "[FennecBlockerExtensionDetector] Re-enable handling failed",
          error
        );
      });
  },
  ...
  _onDetectionDismissedPrefChanged() {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[FennecBlockerExtensionDetector] Failed to re-enable detection notifications",
        error
      );
    });
  },
  ...
  _onCoexistPrefChanged() {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[FennecBlockerExtensionDetector] Failed to refresh after coexist toggle",
        error
      );
    });
  },
  ...
  _removeStartupObserver() {
    ...
    try {
      ...
      console.warn(
        "[FennecBlockerExtensionDetector] Failed to remove startup observer:",
        err
      );
    }
    ...
    const progressListener = {
      onLocationChange: (browser, webProgress, _request, location) => {
        ...
        this._maybeShowDetectionUpgradeForBrowser(win, browser, location).catch(
          error => {
            console.error(
              "[FennecBlockerExtensionDetector] Failed to show upgrade modal",
              error
            );
          }
        );
  ...
  _seedDetectionForWindow(win, source = "seed") {
    ...
    const tryBrowser = browser => {
      this._maybeShowDetectionUpgradeForBrowser(
        ...
        console.error(
          `[FennecBlockerExtensionDetector] Failed to show ${source} upgrade modal`,
          error
        );
      });
    };
  ...
  async _prewarmUpgradeMessage() {
    ...
    this._upgradeMessagePromise =
      lazy.OnboardingMessageProvider.getUpgradeMessage()
        ...
        .catch(error => {
          console.error(
            "[FennecBlockerExtensionDetector] Failed to prewarm upgrade message",
            error
          );
          return null;
        })
  ...
  async _showDetectionUpgradeModal(win, browser, extensionName) {
    ...
    const [spotlightTitle, spotlightPrimaryLabel, spotlightSecondaryLabel] =
      await lazy.blockerLocalization.formatValues([
        "fennec-blocker-spotlight-title",
        "fennec-blocker-spotlight-primary-button",
        "fennec-blocker-spotlight-secondary-button",
      ]);
    const detectedName =
      extensionName ||
    ...
    const spotlightSubtitle = await lazy.blockerLocalization.formatValue(
      "fennec-blocker-spotlight-subtitle",
  ...
  _onInstallStarted(install) {
    ...
    this._preloadLocalizedStrings().catch(error => {
      console.error(
        "[FennecBlockerExtensionDetector] Failed to refresh install warning strings",
        error
      );
    });
  ...
  async _disableAdblockExtensions(addons) {
    ...
    for (const addon of addons) {
      try {
        ...
        failed = true;
        console.error(
          `[FennecBlockerExtensionDetector] Failed to disable ${addon?.id || "<unknown>"}`,
          error
        );
      }
    }
  ...
  async _handleBuiltInBlockerReenabled() {
    ...
    const [promptTitle, messageText, useBuiltInLabel, keepExtensionLabel] =
      await lazy.blockerLocalization.formatValues([
        "fennec-blocker-prompt-title",
        {
          id: "fennec-blocker-reenable-conflict-message",
          args: { extensionName: conflictingName },
        },
        "fennec-blocker-reenable-use-built-in",
        "fennec-blocker-reenable-keep-extension",
      ]);
  ...
  _showInstallWarning(win, addon) {
    try {
      ...
      console.error(
        "[FennecBlockerExtensionDetector] Failed to show install warning prompt",
        error
      );
      return true;
    }
