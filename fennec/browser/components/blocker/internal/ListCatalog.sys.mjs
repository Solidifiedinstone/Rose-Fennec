...
import { CUSTOM_FILTERS_FILE_NAME } from "resource:///modules/FennecBlockerUtils.sys.mjs";

export const CUSTOM_FILTERS_DESCRIPTOR_URL = "fennec://custom-filters";
...
const LIST_CATALOG_URL = "resource://fennec/blocker/assets/list_catalog.json";
const BUNDLED_FILTERS_BASE = "resource://fennec/blocker/assets/filters/";
...
function getCustomFilterListUrls() {
  ...
  if (entries.length && !raw.trim().startsWith("[")) {
    try {
      ...
      console.warn(
        "[FennecBlocker] Failed migrating custom list URL pref:",
        err
      );
    }
  }
