...
const REMOTE_BUNDLES = Object.freeze([
  Object.freeze({
    ...
    bundledUrl:
      "resource://fennec/blocker/assets/resources/ubo-scriptlets.json",
  }),
  Object.freeze({
    name: "resources",
    url: "https://aus.waterfox.com/v2/blocker/resources.json",
    bundledUrl: "resource://fennec/blocker/assets/resources/resources.json",
  }),
]);
...
async function writeMeta(metaPath, meta) {
  try {
    ...
    console.warn(
      `[FennecBlocker] Failed writing remote-resource meta ${metaPath}:`,
      err
    );
  }
}
...
async function refreshOneBundle(bundle) {
  ...
  try {
    ...
    console.warn(
      `[FennecBlocker] Failed to refresh remote bundle "${bundle.name}":`,
      err
    );
    await writeMeta(metaPath, nextMeta);
  }
...
async function readBundledArray(bundledUrl) {
  try {
    ...
    console.warn(
      `[FennecBlocker] Failed reading bundled resource ${bundledUrl}:`,
      err
    );
    return [];
  }
...
async function readRemoteArray(bundle) {
  ...
  try {
    ...
    console.warn(
      `[FennecBlocker] Stored remote bundle "${bundle.name}" is invalid, falling back to bundled copy:`,
      err
    );
    return null;
  }
  ...
  async refresh() {
    ...
    for (const bundle of REMOTE_BUNDLES) {
      try {
        ...
        console.warn(
          `[FennecBlocker] Unexpected error refreshing "${bundle.name}":`,
          err
        );
      }
    }
