...
import { CACHE_ROOT_DIR_NAME } from "resource:///modules/FennecBlockerUtils.sys.mjs";
...
async function clearCacheFiles() {
  try {
    ...
    console.warn("[FennecBlocker] Failed removing engine cache file:", err);
  }

  try {
    await IOUtils.remove(cacheMetaPath(), { ignoreAbsent: true });
  } catch (err) {
    console.warn("[FennecBlocker] Failed removing cache metadata file:", err);
  }
}
