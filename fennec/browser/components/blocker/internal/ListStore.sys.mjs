...
const FENNEC_UNBREAK_FILTERS_FILE_NAME = "fennec-unbreak.txt";
const FENNEC_UNBREAK_FILTERS_URL =
  "resource://fennec/blocker/assets/filters/fennec-unbreak.txt";

let gFennecUnbreakRecord = null;
...
async function readJSON(path, fallbackValue) {
  try {
    ...
    if (!isFileNotFoundError(err)) {
      console.warn(`[FennecBlocker] Failed reading JSON ${path}:`, err);
    }
...
async function readCustomFiltersRecord(customDescriptor) {
  ...
  try {
    ...
    console.warn("[FennecBlocker] Failed reading custom filters:", err);
    return null;
  }
}

async function readFennecUnbreakRecord() {
  if (gFennecUnbreakRecord) {
    return gFennecUnbreakRecord;
  }

  const response = await fetch(FENNEC_UNBREAK_FILTERS_URL, {
    cache: "no-store",
  });
  if (!response.ok) {
  ...
  if (!text.trim()) {
    throw new Error("Fennec unbreak filters were empty");
  }

  gFennecUnbreakRecord = {
    filename: FENNEC_UNBREAK_FILTERS_FILE_NAME,
    text,
    url: FENNEC_UNBREAK_FILTERS_URL,
  };
  return gFennecUnbreakRecord;
}

async function withFennecUnbreakRecord(listRecords) {
  return [...listRecords, await readFennecUnbreakRecord()];
}
    ...
    for (const descriptor of descriptors) {
      ...
      for (const filename of filenames) {
        ...
        try {
          ...
          console.warn(
            `[FennecBlocker] Failed reading stored list ${filename}:`,
            err
          );
        }
      }
  ...
  async readBundledLists(descriptors) {
    ...
    for (const descriptor of descriptors) {
      ...
      try {
        ...
        console.warn(
          `[FennecBlocker] Failed to read bundled list: ${descriptor.bundledUrl}`,
          err
        );
      }
    }
