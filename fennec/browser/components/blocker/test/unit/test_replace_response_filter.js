...
add_setup(async function setup() {
  ...
  FennecBlockerService._registerNetworkObservers();

  registerCleanupFunction(async () => {
    FennecBlockerService._unregisterNetworkObservers();
...
add_task(async function test_replace_response_filter_respects_site_bypass() {
  makeEngine();
  const originalBypass = FennecBlockerService.shouldBypassBlocking;
  FennecBlockerService.shouldBypassBlocking = (candidateDomain, options) =>
    candidateDomain === "127.0.0.1" ||
    originalBypass.call(FennecBlockerService, candidateDomain, options);
  ...
  try {
    ...
    FennecBlockerService.shouldBypassBlocking = originalBypass;
  }
});
