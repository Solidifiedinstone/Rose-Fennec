...
const TEST_URL =
  "https://example.com/browser/fennec/browser/components/blocker/test/browser/file_early_scriptlet_injection.html";
...
async function readProbeState(browser) {
  return SpecialPowers.spawn(browser, [], () => {
    ...
    return {
      diagnostics: win.__fennecBlockerEarlyScriptletDiagnostics || null,
...
add_setup(function setup() {
  originalGetCosmeticResources = FennecBlockerService.getCosmeticResources;
  originalInitializeEngineWithRetry =
    FennecBlockerService._initializeEngineWithRetry;
  originalWhenEngineReady = FennecBlockerService.whenEngineReady;

  FennecBlockerService._initializeEngineWithRetry = async function () {};
  FennecBlockerService.getCosmeticResources = function (url) {
  ...
  FennecBlockerService.whenEngineReady = async function () {
  ...
  registerCleanupFunction(() => {
    FennecBlockerService.getCosmeticResources = originalGetCosmeticResources;
    FennecBlockerService._initializeEngineWithRetry =
      originalInitializeEngineWithRetry;
    FennecBlockerService.whenEngineReady = originalWhenEngineReady;
...
add_task(async function test_latency_diagnostics() {
  await withDiagnostics(async () => {
    ...
    info(
      `FennecBlocker early parser block median no-scriptlet duration: ${median(
        noScriptletDurations
      )}ms`
    );
    info(
      `FennecBlocker early parser block median scriptlet duration: ${median(
        scriptletDurations
      )}ms`
    );
  }, [earlyScriptletTimeoutPref()]);
