...
const { FennecBlockedPageChild } = ChromeUtils.importESModule(
  "resource:///modules/FennecBlockedPageChild.sys.mjs"
);
...
const PERMISSION_TYPE = "fennec-blocker";
const PERMISSION_TYPE_PB = "fennec-blocker-pb";
...
function cleanupBlockerState() {
  ...
  FennecBlockerService._clearTopLevelNavigationState();
}
...
function allowIfRecorded(browserId, url) {
  ...
  if (!FennecBlockerService.wasHostBlockedFor(browserId, hostname, url)) {
    return false;
  }

  FennecBlockerService.allowSiteForSession(hostname);
  return true;
}
...
add_task(function test_session_bypass_requires_recorded_blocked_host() {
  ...
  const originalSiteExceptionsState =
    FennecBlockerService._siteExceptionsState;
  FennecBlockerService._siteExceptionsState = makeSiteExceptionsState();

  try {
    FennecBlockerService._rememberBlockedTopLevelDocument(
    ...
    Assert.equal(
      FennecBlockerService.shouldBypassBlocking(FORGED_HOST),
      false,
      "The unrelated host should not receive a session bypass"
    );
    Assert.equal(
      FennecBlockerService.shouldBypassBlocking(BLOCKED_HOST),
      false,
    ...
    FennecBlockerService._clearTopLevelNavigationState();
    FennecBlockerService._siteExceptionsState = originalSiteExceptionsState;
    cleanupBlockerState();
  }
});
...
add_task(async function test_genuine_load_anyway_grants_session_exception() {
  ...
  try {
    ...
    await TestUtils.waitForCondition(
      () =>
        FennecBlockerService.isSiteExcepted(BLOCKED_HOST, {
          isPrivate: false,
        }),
    ...
    Assert.ok(
      FennecBlockerService.isSiteExcepted(BLOCKED_HOST, { isPrivate: false }),
      "The genuine blocked page should grant a normal session exception"
    );
    Assert.ok(
      !FennecBlockerService.isSiteExcepted(BLOCKED_HOST, { isPrivate: true }),
...
add_task(async function test_private_panel_exception_uses_private_scope() {
  ...
  let tab = null;
  const originalReloadCurrentTab = FennecBlockerPanel._reloadCurrentTab;
  FennecBlockerPanel._reloadCurrentTab = () => {};
  ...
  try {
    ...
    FennecBlockerPanel._setSiteExceptionForCurrentSite(privateWin, true);

    Assert.ok(
      FennecBlockerService.isSiteExcepted(PANEL_TEST_HOST, {
        isPrivate: true,
      }),
    ...
    Assert.ok(
      FennecBlockerService.shouldBypassBlocking(PANEL_TEST_HOST, {
        isPrivate: true,
      }),
      "The private panel allow action should bypass blocking in private windows"
    );

    FennecBlockerPanel._setSiteExceptionForCurrentSite(privateWin, false);
    ...
    FennecBlockerService.addSiteException(PANEL_TEST_HOST);
    assertPermission(
      PANEL_TEST_HOST,
      PERMISSION_TYPE,
      true,
    ...
    FennecBlockerService.removeSiteException(PANEL_TEST_HOST);
    assertPermission(
      PANEL_TEST_HOST,
      PERMISSION_TYPE,
      false,
    ...
    FennecBlockerPanel._reloadCurrentTab = originalReloadCurrentTab;
    if (tab) {
      await BrowserTestUtils.removeTab(tab);
    }
    if (privateWin) {
...
add_task(
  async function test_permanent_exception_persists_after_private_session() {
    ...
    try {
      FennecBlockerService.addSiteException(PERMANENT_HOST);
      Assert.ok(
        FennecBlockerService.isSiteExcepted(PERMANENT_HOST, {
          isPrivate: false,
        }),
...
add_task(async function test_forged_blocked_page_does_not_grant() {
  ...
  try {
    ...
    Assert.ok(
      !FennecBlockerService.isSiteExcepted(FORGED_HOST),
...
add_task(function test_untrusted_click_handler_does_not_send_query() {
  ...
  FennecBlockedPageChild.prototype.handleEvent.call(actor, {
    button: 0,
    isTrusted: false,
