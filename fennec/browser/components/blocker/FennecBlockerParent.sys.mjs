 ...
 */
export class FennecBlockerParent extends JSWindowActorParent {
  /**
  ...
  receiveMessage(message) {
    switch (message.name) {
      case "FennecBlocker:GetCosmeticResources":
        return this._getCosmeticResources(message.data);
      case "FennecBlocker:GetHiddenClassIdSelectors":
  ...
  async _getCosmeticResources({ url } = {}) {
    ...
    await FennecBlockerService.whenEngineReady();

    const resources = FennecBlockerService.getCosmeticResources(
      url,
  ...
  _getHiddenClassIdSelectors({ classes, ids, exceptions } = {}) {
    return FennecBlockerService.getHiddenClassIdSelectors(
      classes || [],
      ids || [],
      exceptions || []
    );
