...
const {
  ...
  FennecBlockerService,
  normalizeCustomFiltersText,
} = ChromeUtils.importESModule(
  "resource:///modules/FennecBlockerService.sys.mjs"
);

const CUSTOM_FILTERS_DEFAULT_FILE_NAME = "fennec-custom-filters.txt";
...
var gFennecBlockerCustomFilters = {
  _acceptButton: null,
  _dialog: null,
  _exportButton: null,
  _importButton: null,
  ...
  async init() {
    ...
    this._textarea = document.getElementById(
      "fennecBlockerCustomFiltersText"
    );
    this._status = document.getElementById(
      "fennecBlockerCustomFiltersStatus"
    );
    this._importButton = document.getElementById(
      "fennecBlockerCustomFiltersImport"
    );
    this._exportButton = document.getElementById(
      "fennecBlockerCustomFiltersExport"
    );
    ...
    this._importButton?.addEventListener("command", () => {
      this._importFromFile().catch(err => {
        console.error(
          "[FennecBlocker] Failed to import custom filters:",
          err
        );
        showAlert(
          "fennec-blocker-custom-filters-import-error-title",
          "Import failed",
          "fennec-blocker-custom-filters-import-error",
    ...
    this._exportButton?.addEventListener("command", () => {
      this._exportToFile().catch(err => {
        console.error(
          "[FennecBlocker] Failed to export custom filters:",
          err
        );
        showAlert(
          "fennec-blocker-custom-filters-export-error-title",
          "Export failed",
          "fennec-blocker-custom-filters-export-error",
    ...
    document.addEventListener("dialogaccept", event => {
      ...
      this._saveAndClose().catch(err => {
        console.error("[FennecBlocker] Failed to save custom filters:", err);
        showAlert(
          "fennec-blocker-custom-filters-save-error-title",
          "Save failed",
          "fennec-blocker-custom-filters-save-error",
  ...
  async _load() {
    this._setBusy(true);
    try {
      this._initialText = await FennecBlockerService.getCustomFiltersText();
      ...
      console.error("[FennecBlocker] Failed to load custom filters:", err);
      await showAlert(
        "fennec-blocker-custom-filters-load-error-title",
        "Load failed",
        "fennec-blocker-custom-filters-load-error",
  ...
  _updateStatus() {
    ...
    this._updateStatusLocalized(activeCount, dirty, generation).catch(err => {
      ...
      console.error(
        "[FennecBlocker] Failed to localize custom filters status:",
        err
      );
  ...
  async _updateStatusLocalized(activeCount, dirty, generation) {
    ...
    if (dirty) {
      const unsavedStatus = await formatValue(
        "fennec-blocker-custom-filters-status-unsaved",
        "Unsaved changes."
      );
  ...
  async _saveAndClose() {
    ...
    try {
      ...
      await FennecBlockerService.setCustomFiltersText(normalized);
  ...
  async _importFromFile() {
    const title = await formatValue(
      "fennec-blocker-custom-filters-import-picker-title",
    ...
    if (this._textarea.value !== this._initialText) {
      const confirmTitle = await formatValue(
        "fennec-blocker-custom-filters-import-replace-title",
        "Replace current custom filters?"
      );
      const confirmMessage = await formatValue(
        "fennec-blocker-custom-filters-import-replace-message",
  ...
  async _exportToFile() {
    const title = await formatValue(
      "fennec-blocker-custom-filters-export-picker-title",
...
document.addEventListener("DOMContentLoaded", () => {
  gFennecBlockerCustomFilters.init();
});
