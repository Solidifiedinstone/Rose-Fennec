...
const CONTRACT_ID = "@waterfox.com/fennec-blocker-engine;1";

// Prefs
const PREF_ENABLED = "fennec.blocker.enabled";
const PREF_ALLOW_SEARCH_PARTNER_ADS = "fennec.blocker.allowSearchPartnerAds";
const PREF_FILTER_LIST_URLS = "fennec.blocker.filterListUrls";
const PREF_ENABLED_LISTS = "fennec.blocker.enabledLists";
const PREF_LEGACY_SITE_EXCEPTIONS = "fennec.blocker.siteExceptions";
const PREF_SITE_EXCEPTIONS_MIGRATED =
  "fennec.blocker.siteExceptions.migrated";
const PREF_REMOTE_RESOURCES_ENABLED = "fennec.blocker.remoteResourcesEnabled";
const PREF_GLOBAL_STATS = "fennec.blocker.globalStats";
const PREF_DOMAIN_EXCEPTIONS = "fennec.blocker.domainExceptions";
...
function applyReplaceDirectives(text, directives) {
  ...
  for (const directive of directives) {
    try {
      ...
      console.warn(
        "[FennecBlocker] Skipping invalid $replace directive:",
        err
      );
    }
  }
...
function applyHtmlFilters(text, filters, documentUrl, mimeType) {
  ...
  for (const rawFilter of filters) {
    ...
    try {
      ...
      console.warn("[FennecBlocker] Skipping invalid HTML filter:", err);
      continue;
    }
  ...
  onStopRequest(request, statusCode) {
    if (!this.passThrough) {
      ...
      try {
        ...
        console.warn("[FennecBlocker] Failed to filter response body:", err);
        this.emitBytes(request, originalBytes);
      }
    }
 ...
 */
export const FennecBlockerService = {
  QueryInterface: ChromeUtils.generateQI([
    "nsIObserver",
  ...
  _createEngine() {
    return Cc[CONTRACT_ID].createInstance(Ci.nsIFennecBlockerEngine);
  },
  ...
  async _preprocessListRecords(listRecords) {
    const records = await lazy.ListStore.withFennecUnbreakRecord(listRecords);
  ...
  async _doInitializeEngineIfNeeded() {
    ...
    try {
      ...
      if (e.result !== Cr.NS_ERROR_FILE_NOT_FOUND) {
        console.error("[FennecBlocker] Unexpected cache error:", e);
      }
    }
  ...
  _notifyBlockedCountsCleared() {
    try {
      ...
      console.warn("[FennecBlocker] Failed to notify cleared counts:", err);
    }
  },
  ...
  _notifyBlockedCountUpdated(browserId, blockedCount) {
    try {
      ...
      console.warn("[FennecBlocker] Failed to notify blocked count:", err);
    }
  },
  ...
  _handleTopLevelDocumentRequest(
    ...
    try {
      ...
      console.error(
        "[FennecBlocker] Failed to redirect to blocked page:",
        err
      );
    ...
    try {
      ...
      console.warn("[FennecBlocker] Failed to increment blocked count:", err);
    }
  },
  ...
  _onModifyRequest(subject) {
    ...
    if (result.matched && !result.exception) {
      ...
      try {
        ...
        console.warn(
          "[FennecBlocker] Failed to increment blocked count:",
          err
        );
      }
      return;
  ...
  _redirectChannel(channel, target) {
    try {
      ...
      console.warn("[FennecBlocker] Failed to redirect channel:", err);
      return false;
    }
  },
  ...
  _onExamineResponse(subject) {
    ...
    try {
      ...
      console.error("[FennecBlocker] Failed to apply CSP directives:", err);
    }
  },
  ...
  _maybeFilterResponseBody(
    ...
    try {
      ...
      console.warn("[FennecBlocker] Failed to install response filter:", err);
    }
  },
  ...
  _startListUpdateTriggers() {
    ...
    this._fallbackUpdateIntervalId = lazy.setInterval(() => {
      this._updateListsIfNeeded().catch(err => {
        console.warn("[FennecBlocker] Fallback list update failed:", err);
      });
  ...
  _stopListUpdateTriggers() {
    if (this._listUpdateObserverRegistered) {
      try {
        ...
        console.warn(
          "[FennecBlocker] Failed to remove RS poll observer:",
          err
        );
      }
  ...
  async _refreshEngineAfterListUpdate(anyUpdated, descriptors, generation) {
    ...
    if (anyUpdated) {
      ...
      if (!complete) {
        ...
        console.warn(
          `[FennecBlocker] ${missingCount} bundled filter list(s) unavailable after update; keeping the current engine`
        );
  ...
  async _runListUpdatePass() {
    ...
    try {
      ...
      console.warn("[FennecBlocker] Remote resource refresh failed:", err);
    }
  ...
  checkRequest(
    ...
    try {
      ...
      console.error("[FennecBlocker] checkRequest failed:", err);
      return this._normalizeCheckResult(null);
    }
  },
  ...
  getCosmeticResources(url, browsingContext = null) {
    ...
    try {
      ...
      console.error("[FennecBlocker] getCosmeticResources failed:", err);
      return null;
    }
  ...
  getCspDirectives(
    ...
    try {
      ...
      console.error("[FennecBlocker] getCspDirectives failed:", err);
      return "";
    }
  },
  ...
  getReplaceDirectives(
    ...
    try {
      ...
      console.error("[FennecBlocker] getReplaceDirectives failed:", err);
      return [];
    }
  },
  ...
  getHtmlFilteringResources(url) {
    ...
    try {
      ...
      console.error("[FennecBlocker] getHtmlFilteringResources failed:", err);
      return { htmlFilters: [], responseHeaderFilters: [] };
    }
  },
  ...
  async setCustomFiltersText(text) {
    ...
    if (hadPreviousFile) {
      try {
        ...
        console.warn(
          "[FennecBlocker] Failed reading previous custom filters for rollback:",
          err
        );
      }
    }
    ...
    try {
      ...
      try {
        ...
        console.error(
          "[FennecBlocker] Failed rolling back custom filters after rebuild failure:",
          rollbackErr
        );
      }
  ...
  getHiddenClassIdSelectors(classes = [], ids = [], exceptions = []) {
    ...
    try {
      ...
      console.error("[FennecBlocker] getHiddenClassIdSelectors failed:", err);
      return [];
    }
  },
  ...
  _flushGlobalStats() {
    ...
    try {
      ...
      console.warn("[FennecBlocker] Failed to persist global stats:", err);
    }
  },
  ...
  _saveDomainExceptions() {
    ...
    try {
      ...
      console.warn(
        "[FennecBlocker] Failed to persist domain exceptions:",
        err
      );
    }
  },
  ...
  _unregisterNetworkObservers() {
    ...
    for (const topic of [
      ...
      try {
        ...
        console.warn(
          `[FennecBlocker] Failed to remove observer for ${topic}:`,
          err
        );
      }
    }
  ...
  async _initializeEngineWithRetry() {
    try {
      ...
      if (this.isEnabled()) {
        ...
        this._updateListsIfNeeded().catch(err => {
          console.warn("[FennecBlocker] Initial list update failed:", err);
        });
      }
    } catch (err) {
      console.error("[FennecBlocker] Failed to initialise engine:", err);
      this._scheduleInitRetry();
    }
  },
  ...
  _migrateSiteExceptions() {
    ...
    if (raw) {
      try {
        ...
        console.warn(
          "[FennecBlocker] Failed to parse legacy site exceptions pref:",
          err
        );
      }
    }
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        console.warn(
          `[FennecBlocker] Failed to migrate site exception "${entry}":`,
          err
        );
      }
    }
  ...
  async init() {
    ...
    lazy.EngineCache.cleanupStale().catch(err => {
      console.warn("[FennecBlocker] Cache cleanup failed:", err);
    });
  ...
  observe(subject, topic, data) {
    ...
    if (topic === REMOTE_SETTINGS_POLL_END_TOPIC) {
      this._updateListsIfNeeded().catch(err => {
        console.warn(
          "[FennecBlocker] List update on RS poll-end failed:",
          err
        );
      });
      return;
    ...
    switch (data) {
      ...
      case PREF_FILTER_LIST_URLS:
        if (this.isEnabled()) {
          this.refreshListsAndEngine().catch(err => {
            console.error(
              "[FennecBlocker] Failed to refresh lists after pref change:",
              err
            );
          });
        }
      ...
      case PREF_ENABLED_LISTS:
        if (this.isEnabled()) {
          this.refreshListsAndEngine().catch(err => {
            console.error(
              "[FennecBlocker] Failed to refresh lists after list toggle:",
              err
            );
          });
        }
      ...
      case PREF_REMOTE_RESOURCES_ENABLED:
        if (this.isEnabled() && this._engine) {
          this._loadResourcesAndBumpGeneration().catch(err => {
            console.error(
              "[FennecBlocker] Failed to reload resources after remoteResourcesEnabled change:",
              err
            );
          });
        }
  ...
  uninit() {
    ...
    try {
      ...
      console.warn("[FennecBlocker] Failed to remove pref observer:", err);
    }
