...
import { FennecBrowserStyle } from "resource:///modules/FennecBrowserStyle.sys.mjs";
...
// Lepton modes: 0/1 load the Fennec chrome customisations, 2 turns them off.
const LEPTON_ON = 1;
const LEPTON_OFF = 2;
...
// The theme drives Lepton: only Photon uses the Fennec chrome tab styling;
// Nova and Proton run on stock Firefox chrome (Lepton off).
function setStyle(style) {
  switch (style) {
    case "nova":
      FennecBrowserStyle.applyStockTabStyle();
    ...
    case "proton":
      ...
      break;
    case "photon":
      FennecBrowserStyle.applyPhotonTabStyle();
      Services.prefs.setStringPref(BROWSER_STYLE_PREF, style);
...
function setThemeMode(mode) {
  FennecThemeColors.setMode(mode);
}

function setThemeColor(color) {
  FennecThemeColors.setColor(color);
}
...
function keepPrivacyDefaults() {
  Services.prefs.setBoolPref("fennec.blocker.enabled", true);
}

export const FennecOnboardingActions = {
  async handle(data = {}) {
    switch (data.action) {
      case "style":
        setStyle(data.value);
