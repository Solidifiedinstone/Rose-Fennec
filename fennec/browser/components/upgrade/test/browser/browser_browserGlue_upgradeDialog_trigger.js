...
const { FennecUpgradeMessage } = ChromeUtils.importESModule(
  "resource:///modules/FennecUpgradeMessage.sys.mjs"
);
...
add_task(async function show_major_upgrade() {
  ...
  try {
    ...
    const data = await FennecUpgradeMessage.getUpgradeMessage();
    Assert.equal(
      data.id,
      "FENNEC_153_UPGRADE",
      "Fennec 153 upgrade dialog shown"
    );
    Assert.equal(
      Services.prefs.getIntPref("browser.startup.upgradeDialog.version"),
      FennecUpgradeMessage.dialogVersion,
      "Fennec upgrade dialog version was recorded"
    );
    win.close();
