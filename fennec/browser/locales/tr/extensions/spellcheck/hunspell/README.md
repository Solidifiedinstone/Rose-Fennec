# Hunspell Turkish Dictionary for Fennec

A highly accurate Turkish spellchecking dictionary (`.aff` / `.dic`) optimized, compressed, and packaged specifically for use in **Fennec** as a browser extension.

This project packages state-of-the-art Turkish dictionary data into a legacy/WebExtension format (`.xpi`) so it can be directly loaded into Fennec for seamless, native browser spellchecking.
...
*   **File Size Reduction:** Stripped redundant word stems, duplicated flag combinations, and obsolete word definitions. This drastically reduces memory overhead when Fennec loads the dictionary into RAM.
...
## 🚀 How to Install in Fennec

Because this is optimized specifically as a Fennec extension, you can load it directly into your browser:

1. Open **Fennec**.
2. Type `about:debugging` in the URL address bar and press **Enter**.
3. Click on **"This Fennec"** in the left sidebar.
...
The underlying dictionary data and source files are licensed under the **MPL-2.0 (BrowserWorks Public License 2.0)**. See the `LICENSE` file for more details.