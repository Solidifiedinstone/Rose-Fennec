...
preferences-doh-ultra-desc = { -brand-short-name } будет использовать безопасный DNS с Oblivious HTTP через приватный ретранслятор Fennec для максимальной защиты.

preferences-doh-ultra-detailed-desc-1 = Использовать ретранслятор Fennec Oblivious HTTP, чтобы скрыть DNS-запросы даже от провайдера DNS
