...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-firefox2 = Включите отладку по USB в Fennec на вашем Android-устройстве.
# USB section step by step guide
...
about-debugging-browser-version-too-old-fennec = Эта версия Fennec не может производить отладку Fennec для Android (68). Мы рекомендуем установить Fennec для Android Nightly на телефон для тестирования. <a>Подробнее</a>
...
about-debugging-browser-version-too-recent = Подключённый браузер имеет более новый ({ $runtimeVersion }, buildID { $runtimeID }), чем ваш { -brand-shorter-name } ({ $localVersion }, buildID { $localID }). Эта конфигурация не поддерживается и может привести к сбою в работе Инструментов разработчика. Пожалуйста, обновите Fennec. <a>Решение проблем</a>
