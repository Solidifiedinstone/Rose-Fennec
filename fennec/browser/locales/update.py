...
class LocaleUpdater:
    ...
    def process_files(self, locale_dir):
        ...
        files_processed = 0
        files_modified = 0
        fennec_dir = locale_dir / "browser" / "browser" / "fennec"

        for file_path in locale_dir.rglob("*"):
            if not file_path.is_file():
                continue

            if fennec_dir in file_path.parents:
                continue
    ...
    def preserve_fennec_files(self, temp_locale_dir, out_name):
        """Preserve existing Fennec localisation files"""
        existing_fennec = (
            self.root_dir / out_name / "browser" / "browser" / "fennec"
        )
        if existing_fennec.is_dir():
            new_fennec = temp_locale_dir / "browser" / "browser" / "fennec"
            if new_fennec.exists():
                shutil.rmtree(new_fennec)
            shutil.copytree(existing_fennec, new_fennec)
            print(f"Preserved Fennec localisation files for {out_name}")
    ...
    def update_locale(self, locale_code):
        ...
        try:
            # Preserve existing Fennec localisation files
            self.preserve_fennec_files(temp_locale_dir, out_name)
    ...
    def restructure_branding(self, locale_dir):
        """Move branding files from browser/branding/official to fennec/browser/branding"""
        old_branding_path = locale_dir / "browser" / "branding" / "official"

        if old_branding_path.exists():
            # Create new branding directory structure
            new_branding_path = locale_dir / "fennec" / "browser" / "branding"
