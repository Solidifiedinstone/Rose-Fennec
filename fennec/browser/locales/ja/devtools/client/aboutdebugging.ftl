...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-firefox2 = Android 端末上の Fennec で USB デバッグを有効にします。
# USB section step by step guide
...
about-debugging-browser-version-too-old-fennec = このバージョンの Fennec は Android 版 Fennec (68) をデバッグできません。携帯端末でのテストには Android 版 Fennec Nightly のインストールをお勧めします。<a>詳細</a>
...
about-debugging-browser-version-too-recent = 接続したブラウザー ({ $runtimeVersion }, ビルド ID { $runtimeID }) が、実行中の { -brand-shorter-name } ({ $localVersion }, ビルド ID { $localID }) よりも新しいです。この組み合わせはサポートされてないため、開発ツールでエラーが発生する可能性があります。Fennec を更新してください。<a>トラブルシューティング</a>
