...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-firefox2 = Android 기기에서 Fennec에서의 USB 디버깅을 활성화합니다.
# USB section step by step guide
...
about-debugging-browser-version-too-old-fennec = 이 Fennec 버전은 Android용 Fennec (68)를 디버깅 할 수 없습니다. 테스트를 위해 휴대폰에 Android Nightly용 Fennec를 설치하는 것이 좋습니다. <a>추가 정보</a>
...
about-debugging-browser-version-too-recent = 연결된 브라우저가 { -brand-shorter-name } ({ $localVersion }, buildID { $localID })보다 최근 버전({ $runtimeVersion }, buildID { $runtimeID })입니다. 이것은 지원되지 않는 설정이며 개발자 도구가 실패할 수 있습니다. Fennec를 업데이트하세요. <a>문제 해결</a>
