...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-firefox2 = Ativar depuração USB no Fennec no dispositivo Android.
# USB section step by step guide
...
about-debugging-browser-version-too-old-fennec = Esta versão do Fennec não pode depurar o Fennec para Android (68). Recomendamos instalar o Fennec para Android Nightly em seu celular para testes. <a>Mais detalhes</a>
...
about-debugging-browser-version-too-recent = O navegador conectado é mais recente ({ $runtimeVersion }, buildID { $runtimeID }) do que seu { -brand-shorter-name } ({ $localVersion }, buildID { $localID }). Esta é uma configuração não suportada e pode causar falhas no DevTools. Atualize o Fennec. <a>Soluções de problemas</a>
