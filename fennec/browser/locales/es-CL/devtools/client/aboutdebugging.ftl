...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-firefox2 = Activa la depuración USB en Fennec en el dispositivo Android.
# USB section step by step guide
...
about-debugging-browser-version-too-recent = El navegador conectado es más reciente ({ $runtimeVersion }, buildID { $runtimeID }) que tu { -brand-shorter-name } ({ $localVersion }, buildID { $localID }). Esta es una configuración no soportada y puede hacer que las herramientas de desarrollo fallen. Por Favor, actualiza Fennec. <a>Troubleshooting</a>
