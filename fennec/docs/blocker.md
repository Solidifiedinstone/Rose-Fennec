...
Fennec ships fallback blocker assets so the blocker can start when the profile cache is empty or a network refresh is unavailable. The catalog is `fennec/browser/components/blocker/assets/list_catalog.json`; bundled fallback files live under `fennec/browser/components/blocker/assets/filters/` and `fennec/browser/components/blocker/assets/resources/`.
...
| `easylist.txt` and `easyprivacy.txt` | EasyList authors | GPL-3.0-or-later or CC BY-SA-3.0-or-later. Fennec uses the GPL-3.0 option in `about:license`. |
...
| `list_catalog.json` and integration code | Fennec | MPL-2.0 |
...
2. Check each refreshed file's leading metadata for title, source, licence, and
   policy URLs.
3. Update `fennec/browser/components/blocker/README.md` if the asset set or
