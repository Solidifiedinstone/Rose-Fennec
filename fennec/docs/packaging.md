...
Release deb and rpm packages are built by the openSUSE Build Service (OBS), not by GitHub Actions. The recipe lives in `fennec/browser/installer/linux/obs/`; its README has the detail.

OBS wraps the prebuilt Linux tarball that GitHub Actions builds, signs with the Widevine VMP key and uploads to the CDN. OBS does not compile Fennec and never sees the VMP key. It installs the signed files into deb and rpm packages, signs the packages and repositories with the OBS project key, and hosts the apt and yum repositories.
