...
The Fennec release workflows live under `.github/workflows/`:
...
The workflows reuse the infrastructure and secret names from the previous Fennec release pipeline. The signing helpers and keys are decoded from secrets at runtime and are not stored in the source tree.
...
- OBS workflow token trigger with `OBS_WORKFLOW_TOKEN_ID` and
  ...
  `download_files` downloads `fennec-<version>.tar.bz2`, and `set_version`
