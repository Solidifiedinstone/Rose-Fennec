...
Fennec uses the Mozilla build system. The usual Firefox setup documentation and `./mach` commands apply. Fennec adds its own mozconfigs and product files to the upstream tree.

## Mozconfigs

The shared configuration lives in `fennec/build/mozconfig.common`. The entry files are small parameter manifests that source it:
...
- `fennec/build/mozconfig-x86_64-pc-linux-gnu` for Linux x64 release builds.
- `fennec/build/mozconfig-aarch64-unknown-linux-gnu` for Linux ARM64 release builds.
- `fennec/build/mozconfig-x86_64-pc-windows-msvc` for Windows x64 cross builds.
- `fennec/build/mozconfig-aarch64-pc-windows-msvc` for Windows ARM64 cross builds.
- `fennec/build/mozconfig-x86_64-apple-darwin` and `fennec/build/mozconfig-aarch64-apple-darwin` for macOS builds.

CI selects a target with `MOZCONFIG=fennec/build/mozconfig-<triple>`. The release and prerelease paths are selected with the `WFX_RELEASE` and `WFX_PRE_RELEASE` environment variables. Those modes enable the Fennec release settings in the mozconfigs, including official build flags and Fennec update configuration. The manifests pass target specifics to the shared fragment through lowercase `wfx_*` shell variables; keep them lowercase, because configure imports uppercase shell variables from the mozconfig.
...
The script reads `FennecSettingsPolicy.requiredOfflineDumps` and updates the source dumps from Mozilla Remote Settings. Fennec's search configuration and icons are static browser resources and are not part of this refresh.
